...
const BulkTagProcessor: React.FC<BulkTagProcessorProps> = ({ open, onClose, onComplete }) => {
  ...
  const [customSourceCategories, setCustomSourceCategories] = useState<{id: string, label: string}[]>([]);
            ...
            <TabsContent value="sources" className="space-y-4 py-4">
              <div className="space-y-4">
                <Alert>
                  ...
                  <AlertTitle>Source Categories</AlertTitle>
                  <AlertDescription>
                    View and manage available source categories for transcripts
                  </AlertDescription>
                </Alert>
                
                <div className="border rounded-md p-4">
                  <h3 className="text-sm font-medium mb-2">Available Source Categories:</h3>
                  <div className="grid grid-cols-1 gap-2 mb-4">
                    {sourceCategories.map(category => (
                      <div key={category.id} className="flex items-center justify-between bg-muted/30 rounded p-2 px-3">
                  ...
                  <Alert variant="outline" className="mt-4">
                    <AlertCircle className="h-4 w-4" />
                    <AlertDescription className="text-xs">
                      Source categories are used to organize and filter transcripts by their origin. 
                      When processing transcripts, the system can auto-detect the source category
                      or apply a default source to all transcripts.
                    </AlertDescription>
                  </Alert>
                </div>
              </div>
