...
import { defineGenkitFunction, getModel, getEmbedder, defineFlow, runFlow } from '@genkit-ai/firebase/functions';
...
import { index, retrieve, upsert } from '@genkit-ai/ai/vector';
...
import pdf from 'pdf-parse'; // Requires `npm install pdf-parse`
...
const ingestAndIndexFlow = defineFlow(
    {
        ...
        inputSchema: z.object({ filePath: z.string(), bucket: z.string(), uid: z.string(), sourceName: z.string() }),
        outputSchema: z.void(),
    },
...
Summary:`; // Limit context for summary
...
export const ingestTranscript = onObjectFinalized(
    { cpu: 1, memory: '1GiB', timeoutSeconds: 540, region: 'us-central1', bucket: process.env.GCLOUD_BUCKET || '', // Set bucket via env var
